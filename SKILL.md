...
## 輸入處理
- **本地文件**: 直接讀取文件路徑
- **YouTube**: 使用 `download.sh` 腳本下載（基於 yt-dlp）
...
- 根據內容提供適當的分類標籤
- **所有總結開頭必須標明用戶提供的原始視頻連結**

# 背景運行模式
本 Skill 支援以 Subagent 方式在背景運行，任務完成或出現錯誤時才通知用戶。

## 使用方式
用戶提供視頻連結/檔案後，明確告知要以 Subagent 方式運行，例如：
> 「幫我分析這個視頻，以 subagent 方式在背景運行」

## Subagent 職責
- 在背景執行完整的視頻分析流程（下載 → 提取幀 → 音頻轉錄 → 分析 → 生成總結）
- **不主动向用戶發送中間進度消息**
- 完成後輸出最終結構化總結
- 出現錯誤時報告錯誤詳情
